...
def eval_latest_model(folder, writer, global_key, state, fn_eval, prefix="checkpoint_"):
    state = checkpoints.restore_checkpoint(folder, state, prefix=prefix)
...
def train_loop(
    ...
    if os.path.exists(config.get("model_init_folder", "")):
        state = checkpoints.restore_checkpoint(config.model_init_folder, state)
        logging.info(
            "Restored from %s at step %d", config.model_init_folder, state.step
        )
