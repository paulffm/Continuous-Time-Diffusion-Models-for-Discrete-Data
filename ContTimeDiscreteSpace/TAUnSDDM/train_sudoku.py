...
import numpy as np
from lib.datasets.sudoku import sudoku_acc
torch.cuda.empty_cache()

def main():
    train_resume = False
    script_dir = os.path.dirname(os.path.realpath(__file__))
    save_location = os.path.join(script_dir, 'SavedModels/SUDOKU/') #'SavedModels/BIN-MNIST/'
    ...
    while True:
        for minibatch in tqdm(dataloader):
            ...
            if (state["n_iter"] + 1) % cfg.sampler.sample_freq == 0 or state[
                ...
                sudoku_acc(samples, return_array=False)
