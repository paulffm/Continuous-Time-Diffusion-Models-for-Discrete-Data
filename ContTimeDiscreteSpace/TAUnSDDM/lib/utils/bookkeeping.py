...
def save_config_as_yaml(cfg, save_dir):
    existing_configs = sorted(
        glob.glob(Path(save_dir).joinpath("config_*.yaml").as_posix())
    )
    ...
    if len(existing_configs) == 0:
        ...
        save_name = Path(save_dir).joinpath(
            "config_{0:03d}.yaml".format(most_recent_num + 1)
        )
...
def get_most_recent_checkpoint(dir, mapping):
    ...
    all_ckpts = sorted(
        glob.glob(dir.joinpath("checkpoints").joinpath("ckpt_*.pt").as_posix())
    )
...
class NumpyWriter:
    ...
    def save_to_disk(self):
        ...
        for item in self.figure_data:
            name, fig, idx = item
            fig.savefig(
                Path(self.save_dir).joinpath(name + "_" + str(idx) + ".pdf"),
                format="pdf",
            )
...
def signal_handler(
    sig, frame, preemption_log_path, checkpoint_dir, state, num_checkpoints_to_keep
):
...
def check_for_preempted_run(
    save_location, start_date, cfg, prepare_to_resume_after_timeout
):
    ...
    for inner_run_path in reversed(inner_run_paths):
        ...
        if not (
            "SIGCONT" in preemption_log[-1] or "Expecting Timeout" in preemption_log[-1]
        ):
            continue

        # its confusing if there's changes in configs going on as well
        assert not os.path.isfile(
            inner_run_path.joinpath("config").joinpath("config_002.yaml")
        )

        run_config = load_ml_collections(
            inner_run_path.joinpath("config").joinpath("config_001.yaml")
        )
