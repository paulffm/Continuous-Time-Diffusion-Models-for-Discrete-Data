...
class NiN(nn.Module):
    ...
    def forward(
        self, x: TensorType["batch", "in_ch", "H", "W"]
...
class AttnBlock(nn.Module):
    ...
    def forward(
        self, x: TensorType["batch", "channels", "H", "W"]
...
class Downsample(nn.Module):
    ...
    def forward(
        self, x: TensorType["batch", "ch", "inH", "inW"]
...
class UNet(nn.Module):
    ...
    def forward(
        self, x: TensorType["B", "C", "H", "W"], timesteps: TensorType["B"] = None
    ) -> TensorType["B", "twoC", "H", "W"]:
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x: TensorType["B", "L", "K"], temb: TensorType["B", "temb_dim"]):
        B, L, K = x.shape
...
class TransformerEncoder(nn.Module):
        ...
        for i in range(num_layers):
            self.encoder_layers.append(
                TransformerEncoderLayer(
                    d_model, num_heads, dim_feedforward, dropout, 4 * temb_dim
                )
            )
    ...
    def forward(self, x: TensorType["B", "L"], times: TensorType["B"]):
        B, L = x.shape
        ...
        if self.use_cat:
            if self.use_one_hot_input:
                x = self.input_embedding(one_hot_x.float())
            else:
                x = self.input_embedding(x)
        else:
            x = self.normalize_input(x, self.S)
            x = x.view(B, L, 1)
            x = self.input_embedding(x)
...
class ResidualMLP(nn.Module):
    def __init__(
        self, num_layers, d_model, hidden_dim, D, S, time_scale_factor, temb_dim
    ):
        super().__init__()
        ...
        for n in range(num_layers):
            self.layers1.append(nn.Linear(d_model, hidden_dim))
            self.layers2.append(nn.Linear(hidden_dim, d_model))
            self.norm_layers.append(nn.LayerNorm(d_model))
            self.temb_layers.append(nn.Linear(4 * temb_dim, 2 * d_model))
    ...
    def forward(self, x: TensorType["B", "D"], times: TensorType["B"]):
        B, D = x.shape
        S = self.S
        ...
        for n in range(self.num_layers):
            h = self.norm_layers[n](
                h + self.layers2[n](self.activation(self.layers1[n](h)))
            )
