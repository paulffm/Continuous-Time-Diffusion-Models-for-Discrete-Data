...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        query = query / torch.sqrt(
            torch.tensor(query.shape[-1], device=self.device, dtype=torch.float32)
        )
        # B, D, H, HD
        ...
        attn_weights = torch.where(
            joint_mask,
            logits,
            torch.tensor(torch.finfo(logits.dtype).min, device=self.device),
        )  # B, D, H, H*HD
...
class TransformerMlpBlock(nn.Module):  # directly used in FFResidual in TAU
    def __init__(
        ...
        self.fc1 = nn.Linear(embed_dim, mlp_dim, bias=bias_init)
...
# synthetic data set:
class SyntheticHollowTransformer(nn.Module):
    """Transformer in one direction."""

    def __init__(self, config, direction):
        super(SyntheticHollowTransformer, self).__init__()
        self.config = config
        self.device = config.device
        self.direction = direction
        self.embed_dim = config.model.embed_dim
        ...
        self.out_mlp = MLP(
            [self.config.model.embed_dim, config.model.mlp_dim, self.S],
            activation=nn.GELU(),
        )

    def forward(
        self, x: TensorType["B", "D"], t: TensorType["B"], conditioner=None
    ) -> TensorType["B", "D", "E"]:
        temb = transformer_timestep_embedding(
            t * self.temb_scale, self.embed_dim, device=self.device
        )  # B, E
        ...
        x = torch.cat([conditioner, x[:, :-1]], dim=1)
        mask = torch.triu(
            torch.ones((concat_dim, concat_dim), device=self.device, dtype=torch.bool),
            diagonal=1,
        )
