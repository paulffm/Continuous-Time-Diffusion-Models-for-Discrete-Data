...
class TauLDRBackward(torch_backward_model.BackwardModel):
    ...
    def get_logprob(self, params, xt, t, xt_target=None):
        ...
        qt0_denorm = qt0.permute(0, 2, 1)[b, xt]

        qt0_numer = torch_utils.expand_dims(qt0, axis=list(range(1, xt.dim() - 1)))
        inner_sum = torch.matmul((p0t / qt0_denorm), qt0_numer)
    ...
    def loss(self, params, rng, x0, xt, t):
        ...
        cat_dims = xt.shape[1]
        b = torch_utils.expand_dims(torch.arange(bsize), axis=1)
        ...
        outer_qt0_numer_sig = qt0[
            torch.arange(bsize).repeat_interleave(d * s),
            torch.flatten(x0).repeat_interleave(s),
            torch.tile(torch.arange(s), (bsize * d,)),
        ].view(bsize, d, s)

        outer_qt0_denom_sig = (
            qt0[
                torch.arange(bsize).repeat_interleave(d),
                torch.flatten(x0),
                torch.flatten(xtilde),
            ]
            + eps
        )

        qt0_denom_sig = qt0.permute(0, 2, 1)[b, xtilde] + eps

        inner_log_sig = torch.log(torch.matmul((p0t_sig / qt0_denom_sig), qt0) + eps)
        xtilde_onehot = F.one_hot(xtilde, num_classes=s).to(torch.float32)
        outer_rate_sig = rate_mat[
            torch.arange(bsize).repeat_interleave(d * s),
            torch.tile(torch.arange(s), [bsize * d]),
            torch.flatten(xtilde).repeat_interleave(s),
        ].view(bsize, d, s)
        ...
        rate_row_sums = -rate_mat[
            torch.arange(bsize).repeat_interleave(s),
            torch.tile(torch.arange(s), (bsize,)),
            torch.tile(torch.arange(s), (bsize,)),
        ].view(bsize, s)

        base_z_tmp = rate_row_sums[
            torch.arange(bsize).repeat_interleave(d), torch.flatten(xtilde)
        ].view(bsize, d)
