...
class BinaryTransformerScoreFunc(nn.Module):
    ...
    def forward(self, x, t):
        ...
        cls_token = torch.ones((x.size(0), 1), dtype=torch.long) * self.config.vocab_size
...
class CatMLPScoreFunc(nn.Module):
    def __init__(self, vocab_size, cat_embed_size, num_layers, hidden_size, time_scale_factor=1000.0):
...
class BinaryScoreModel(nn.Module):
    ...
    def get_q(self, params, xt, t):
        ...
        qxt = self.net({'params': params}, xt, t)
        mask = torch.eye(ddim, device=xt.device).repeat(bsize, 1)
        xrep = xt.repeat_interleave(ddim, dim=0)
        xneg = (mask - xrep) * mask + (1 - mask) * xrep
        t = t.repeat_interleave(ddim)
        qxneg = self.net({'params': params}, xneg, t)
        qxt = qxt.repeat_interleave(ddim, dim=0)
        return qxneg, qxt

    def get_logits(self, params, xt, t):
        bsize = xt.shape[0]
        qxneg, qxt = self.get_q(params, xt, t)
        qxneg = qxneg.view(bsize, -1)
        qxt = qxt.view(bsize, -1)
...
class CategoricalScoreModel(nn.Module):
    ...
    def get_logits(self, params, xt, t):
        ...
        mask = torch.eye(ddim, dtype=torch.int32).repeat(bsize * vocab_size, 1)
        xrep = xt.repeat_interleave(ddim * vocab_size, dim=0)
        candidate = torch.arange(vocab_size).repeat(bsize)
        candidate = candidate.unsqueeze(1).repeat(1, ddim)
        xall = mask * candidate + (1 - mask) * xrep
        t = t.repeat_interleave(ddim * vocab_size)
        qall = self.net({'params': params}, x=xall, t=t)
        logits = qall.view(bsize, ddim, vocab_size).permute(0, 2, 1)
        return logits
    ...
    def get_logprob(self, params, xt, t):
        ...
        ll_xt = ll_all[torch.arange(bsize)[:, None],
                       torch.arange(ddim)[None, :],
                       xt]
        return ll_all, ll_xt
