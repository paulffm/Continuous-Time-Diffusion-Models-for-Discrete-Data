...
class BinarySyntheticHelper(object):
    ...
    def __init__(self, config):
        self.config = config
        self.bm, self.inv_bm = data_utils.get_binmap(
            config.discrete_dim, config.binmode
        )

    def plot(self, xbin, output_file=None):
        fn_xbin2float = functools.partial(
            data_utils.bin2float,
            inv_bm=self.inv_bm,
            discrete_dim=self.config.discrete_dim,
            int_scale=self.config.int_scale,
        )
...
def main(argv: Sequence[str]) -> None:
    ...
    train_eval.train_loop(
        ...
        fn_plot_data=functools.partial(
            synthetic_common.fn_plot_data,
            config=config,
            model=model_helper,
            writer=writer,
        ),
        fn_eval=functools.partial(
            synthetic_common.fn_eval_with_mmd,
            eval_mmd=eval_mmd,
            config=config,
            model=model_helper,
            writer=writer,
