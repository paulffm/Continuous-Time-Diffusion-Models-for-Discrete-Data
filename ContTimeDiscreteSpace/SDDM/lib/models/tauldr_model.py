...
class TauLDRBackward:
    ...
    def loss(self, params, rng, x0, xt, t):
        ...
        inner_log_sig = jnp.log((p0t_sig / qt0_denom_sig) @ qt0 + eps)
        ...
        oss_tmp = outer_qt0_numer_sig / jnp.reshape(outer_qt0_denom_sig, [bsize, d, 1])
        outer_sum_sig = jnp.sum(
            (1 - xtilde_onehot) * outer_rate_sig * oss_tmp * inner_log_sig, axis=(1, 2)
        )
        ...
        base_z_tmp = rate_row_sums[jnp.repeat(jnp.arange(bsize), d), jnp.ravel(xtilde)]
