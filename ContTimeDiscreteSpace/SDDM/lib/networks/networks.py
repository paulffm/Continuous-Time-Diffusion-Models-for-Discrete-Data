...
def cross_attention(config, l2r_embed, r2l_embed, temb):
    ...
    dense = functools.partial(
        nn.linear.DenseGeneral, axis=-1, features=(config.num_heads, head_dim)
    )
    ...
    x = nn.linear.DenseGeneral(features=(config.embed_dim,), axis=(-2, -1), name="out")(
        x
    )
    return x
...
class TransformerEncoder(nn.Module):
    ...
    def __call__(self, x, temb, conditioner=None):
        ...
        for layer_idx in range(config.num_layers):
            x = TransformerBlock(name="block_{}".format(layer_idx), config=config)(
                x, masks=None
            )
        x = x[:, 1:]
        return x
...
class MaskedTransformer(nn.Module):
    ...
    def __call__(self, x, temb, pos):
        ...
        if config.readout == "mlp":
            logits = MLP([2 * config.embed_dim, config.vocab_size], activation=nn.gelu)(
                embed
            )
