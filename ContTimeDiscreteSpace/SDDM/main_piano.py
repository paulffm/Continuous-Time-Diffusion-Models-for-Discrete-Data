...
class PianoModel(continuous_time_diffusion.CategoricalDiffusionModel):
    ...
    def eval_step(self, state, rng, batch):
        ...
        x0 = self.sample_loop(
            state, rng=rng, num_samples=batch.shape[0], conditioner=conditioner
        )
        ...
        frac_sqr = frac**2
        dist_gen = generate_note_cnt / (
            jnp.sum(generate_note_cnt, axis=1, keepdims=True) + 1e-20
        )
        dist_gt = gt_note_cnt / (jnp.sum(gt_note_cnt, axis=1, keepdims=True) + 1e-20)
        distance = (
            1.0
            / jnp.sqrt(2)
            * jnp.sqrt(jnp.sum((jnp.sqrt(dist_gen) - jnp.sqrt(dist_gt)) ** 2, axis=1))
        )
        aux = {
...
def main(argv: Sequence[str]) -> None:
    ...
    fn_eval = functools.partial(model.eval_loop, test_tf_ds=test_tf_ds, writer=writer)
    if config.phase == "train":
        train_eval.train_loop(
            config,
            writer=writer,
            global_key=global_key,
            train_step_fn=model.step_fn,
            train_ds=train_ds,
            state=state,
            fn_eval=fn_eval,
        )
